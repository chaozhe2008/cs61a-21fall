...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    true_list = [s for s in paragraphs if select(s)]
    if len(true_list) <= k:
        return ''
    else:
        return true_list[k]
...
def about(topic):
    ...
    def about_or_not(s):
        s_lower = s.lower()
        s_temp = [t if (t.isalpha() or t == ' ') else '' for t in s_lower]
        s_substituted = ''.join(s_temp)
        s_words = s_substituted.split()
        for i in topic:
            if i in s_words:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        match_list = [i for i in range(min(len(typed_words), len(reference_words))) if typed_words[i] == reference_words[i]]
        return len(match_list)/len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        dif.append(diff_function(typed_word, i, limit))
    min_dif = min(dif)
    potential_words = [i for i in range(len(valid_words)) if dif[i] == min_dif]
    if min_dif > limit:
        return typed_word
    return valid_words[potential_words[0]]
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == '' or goal == '' or limit < 0:  # Fill in the condition
        ...
        return min(max(len(goal), len(start)), limit + 1)
    elif start[0] == goal[0]:  # Feel free to remove or add additional cases
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    time = [[(i[j+1] - i[j]) for j in range(len(times_per_player[0])-1)] for i in times_per_player]
...
def fastest_words(match):
    ...
    m = len(times)  # number of players
    res = [[] for p in range(m)]
    for i in range(n):
        word = word_at(match, i)
        time_list = [player[i] for player in times]
        for j in range(m):
            if time_list[j] == min(time_list):
                res[j].append(word)
                break
    return res
