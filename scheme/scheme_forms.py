...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        new_procedure = do_lambda_form(Pair(signature.rest, expressions.rest), env)
...
def do_and_form(expressions, env):
    ...
    while is_scheme_true(curr) and expressions.rest != nil:
        expressions = expressions.rest
        curr = scheme_eval(expressions.first, env)
    if not is_scheme_true(curr):
        return False
    else:
        return curr
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    curr = scheme_eval(expressions.first, env) # 避免重复eval同一个expression
    while not is_scheme_true(curr) and expressions.rest != nil:
        expressions = expressions.rest
        curr = scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        validate_form(curr_binding, 2, 2)
        name = curr_binding.first
        value = eval_all(curr_binding.rest, env)
        names = Pair(name, names)
        values = Pair(value, values)
        bindings = bindings.rest
    # END PROBLEM 14
    validate_formals(names)  # check repeat in names
