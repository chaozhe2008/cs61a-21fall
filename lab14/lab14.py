...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        t.branches = sorted(t.branches, key=lambda x: x.label)[:1]
...
class Game:
    ...
    def play(self):
        while not self.game_over:
            ...
            other_player = self.p1 if self.turn % 2 == 1 else self.p2
            player.choose(other_player)(other_player)
            self.turn += 1
        return self.winner
...
def add_trees(t1, t2):
    ...
    if max_l == 0:
        return res
    if l1 > l2:
        t2.branches += ([Tree(0)] * (l1 - l2))
    else:
        t1.branches += ([Tree(0)] * (l2 - l1))
    branch_pairs = zip(t1.branches, t2.branches)
    for pair in branch_pairs:
        res.branches.append(add_trees(*pair))
    return res
