...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        ...
        my_score += take_turn(strategy(my_score, opponent_score), opponent_score, dice, goal)
        my_score += hog_pile(my_score, opponent_score)
        if who == 0:
            score0 = my_score
        else:
            score1 = my_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        new_score = score0 if who == 0 else score1
        running_h = running_high
        gain = new_score - last_score
        if gain > running_high:
            running_h = gain
            print("%d point(s)! That\'s a record gain for Player %d!" % (running_h, who))
        return announce_highest(who, new_score, running_h)
    return say
    # END PROBLEM 7
'''
For Problem 7, following code does not work because it changes the argument of the function
running_high when executing the function.
    ...
    def say_highest(score0, score1):
        ...
        gain = new_score - last_score
        if gain > running_high:
            print("%d point(s)! That\'s a record gain for Player %d!" %(gain,who))
            running_high = gain
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def repeated_called(*args):
        sum = 0
        for i in range(0, trials_count):
            sum += original_function(*args)
        return sum/trials_count
    return repeated_called
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num in range(1,11):
        ...
        average_dic[num] = averaged_dice(num, dice)
    max_value = max(average_dic.values())
    max_num_lis = [key for key, value in average_dic.items() if value == max_value]
    return min(max_num_lis)
    # END PROBLEM 9
