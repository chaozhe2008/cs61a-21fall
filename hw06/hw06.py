...
class Mint:
    ...
    def create(self, kind):
        "*** YOUR CODE HERE ***"
        res = kind(self.year)
        res.present_year = self.present_year
        return res
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            res = VirFib(self.value + self.pre)
        res.pre = self.value
        return res
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            ...
            return min(t.label, min([bst_min(childs) for childs in t.branches]))

    def bst_max(t):
        if t.is_leaf():
            return t.label
        else:
            return max(t.label, max([bst_max(childs) for childs in t.branches]))
    ...
    if len(t.branches) == 1:
        ...
        left = t.branches[0]
        right = t.branches[1]
    return is_bst(t.branches[0]) and is_bst(t.branches[1]) and bst_max(left) <= t.label < bst_min(right)
